...
| Opening-stock preview and replay | CSV validation and approved replay are restricted to tenant `Admin` plus the `Approve` capability; replay requires server-derived approver metadata, stable references, current-tenant masters, and an atomic append-only lineage record. | M03 must add baseline lock/reversal controls and the separately approved M06 journal. |
| Units import and webhook subscriptions | Tenant-wide configuration; restricted to tenant `Admin`. | Add company scope only with a defined ownership and delivery contract. |
