...
| Customers | Tenant-filtered and owned by one company through a composite tenant/company FK. Every list, read, create, update, deactivate and reactivate action checks the target company's `View` or `Edit` grant; foreign-company customer IDs are returned as not found within the authorized company route. | No shared-master or legacy mapping exists. Add a reference-based deactivation guard when a persisted sales/AR document relationship exists. |
| Chart-of-account configuration | Tenant-filtered and company-owned through composite tenant/company and parent-account keys. Listing requires the target company's `View` grant; creation requires `Edit`; submitted company/tenant ownership is ignored. Writes use the shared audit pipeline. | Configuration only: no approved account taxonomy, control-account mappings, financial posting, fiscal periods, or finance-owner/reviewer approval is represented. |
...
Automated tests cover the six capability personas, customer validation/audit and company filtering, company/stock filtering, unmapped transfer rejection, membership revocation, shared-item API gating, forecast cache separation and company-filtered anomaly predicates. PostgreSQL migration, tenant/company foreign-key, uniqueness and delete-restriction evidence is provided only when the opt-in PostgreSQL suite runs; InMemory tests do not qualify those relational guarantees. Do not treat this matrix or passing in-memory tests as production authorization certification.
