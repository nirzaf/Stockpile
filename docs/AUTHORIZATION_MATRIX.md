...
| Opening-stock preview | Read-only CSV validation is restricted to tenant `Admin` plus the `Approve` capability; it queries only current-tenant item and location masters and never posts stock. | M03 must add approved replay, immutable opening lineage and the separately approved M06 journal. |
